<div class="space-y-6">
    <section class="rounded-[2rem] border border-slate-200/80 bg-white/85 p-6 shadow-[0_12px_30px_rgba(15,23,42,0.04)] backdrop-blur">
        <div class="flex items-start justify-between gap-4 mb-6">
            <div>
                <p class="text-xs font-semibold uppercase tracking-[0.22em] text-slate-500">Edit Event</p>
                <h3 class="mt-1 text-2xl font-semibold text-slate-950">Edit: {{ $event->title }}</h3>
            </div>
        ...
        <form wire:submit.prevent="updateEvent" class="space-y-4">
            ...
            <div>
                ...
                @if ($event->image && !$image)
                    <div class="mb-3 rounded-2xl overflow-hidden border border-slate-200">
                        <img src="{{ asset('storage/' . $event->image) }}" alt="{{ $event->title }}" class="w-full max-h-64 object-cover">
                        <p class="mt-2 text-xs text-slate-500 p-2">Nama file: {{ basename($event->image) }}</p>
                    </div>
                @endif
                ...
                @if ($image)
                    <div class="mt-3 rounded-2xl overflow-hidden border border-slate-200">
                        ...
                        <p class="mt-2 text-xs text-slate-500 p-2">Nama file baru: {{ $image->getClientOriginalName() }}</p>
                    </div>
                @endif
            </div>
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                <div>
                    ...
                    <input wire:model="form.ticket_price" type="number" min="0" step="1000" class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm outline-none transition focus:border-slate-400 focus:bg-white">
