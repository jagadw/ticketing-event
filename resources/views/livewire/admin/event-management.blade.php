                    ...
                    <tbody class="divide-y divide-slate-200 bg-white">
                        @forelse ($this->filteredEvents as $event)
                            <tr>
                                ...
                                <td class="px-5 py-4">
                                    <span class="rounded-full px-3 py-1 text-xs font-medium {{ $event['status'] === 'Published' ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700' }}">{{ $event['status'] }}</span>
                                </td>
